...
class MainFacadeService extends FacadeService_1.FacadeService {
    ...
    getConnection(correlationId, callback) {
        this._connectionResolver.resolve(correlationId, (err, con) => {
            ...
            if (connection == null) {
                err = new pip_services_commons_node_2.ConfigException(correlationId, "NO_CONNECTION", "Connection for REST client is not defined");
            }
            else {
                // Check for type
                let protocol = connection.getProtocol("http");
                if ("http" != protocol && "https" != protocol) {
                    err = new pip_services_commons_node_2.ConfigException(correlationId, "WRONG_PROTOCOL", "Protocol is not supported by REST connection")
                        .withDetails("protocol", protocol);
                    // Check for host
                }
                else if (connection.getHost() == null) {
                    err = new pip_services_commons_node_2.ConfigException(correlationId, "NO_HOST", "No host is configured in REST connection");
                    // Check for port
                }
                else if (connection.getPort() == 0) {
                    err = new pip_services_commons_node_2.ConfigException(correlationId, "NO_PORT", "No port is configured in REST connection");
                }
            }
    ...
    getCredential(correlationId, connection, callback) {
        ...
        this._credentialResolver.lookup(correlationId, (err, con) => {
            ...
            // Check for connection
            if (credential == null) {
                err = new pip_services_commons_node_2.ConfigException(correlationId, "NO_CREDENTIAL", "SSL certificates are not configured for HTTPS protocol");
            }
            else {
                if (credential.getAsNullableString('ssl_key_file') == null) {
                    err = new pip_services_commons_node_2.ConfigException(correlationId, "NO_SSL_KEY_FILE", "SSL key file is not configured in credentials");
                }
                else if (credential.getAsNullableString('ssl_crt_file') == null) {
                    err = new pip_services_commons_node_2.ConfigException(correlationId, "NO_SSL_CRT_FILE", "SSL crt file is not configured in credentials");
                }
            }
